...
export async function action(query?: EventsQuery): Promise<EventsProcessed> {
  ...
  try {
    ...
    if (!query?.networkName) {
      loggerHandler.warn(`${name} missing network name query`);
    ...
    const network = await db.networks.findOne({
      where: {
        name: { [Op.iLike]: String(query?.networkName).replaceAll(" ", "-") }
      },
      raw: true,
    });

    if (!network) {
      loggerHandler.warn(`${name} not found network ${query?.networkName}`);
      return eventsProcessed;
    }

    const { networkAddress, id: network_id, name: networkName } = network
